...
void initAdc0Ss2_3()
{
    ...
    // Configure ADC
    ADC0_ACTSS_R &= ~(ADC_ACTSS_ASEN3 | ADC_ACTSS_ASEN2);                // disable sample sequencer 3 (SS3) for programming
    ADC0_CC_R = ADC_CC_CS_SYSPLL;                    // select PLL as the time base (not needed, since default value)
    ADC0_PC_R = ADC_PC_SR_1M;                        // select 1Msps rate
    ADC0_EMUX_R = ADC_EMUX_EM3_PROCESSOR | ADC_EMUX_EM2_PROCESSOR;            // select SS3 and SS2 bit in ADCPSSI as trigger
    ADC0_SSCTL3_R = ADC_SSCTL3_END0 | ADC_SSCTL0_END2;                 // mark first sample as the end
    ADC0_ACTSS_R |= (ADC_ACTSS_ASEN3 | ADC_ACTSS_ASEN2);                 // enable SS3 for operation
}
...
void setAdc0Ss2Mux(uint8_t input)
{
    ...
    ADC0_ACTSS_R |= ADC_ACTSS_ASEN2;				 // Enable
}
